...
public final class WebDriverBackedEmbeddedBrowser implements EmbeddedBrowser {
	...
	public void goToUrl(URI url) {
		try {
			...
			//browser.navigate().refresh(); // Get fragments 
	...
	public ArrayList<JavaScriptError> getCurrentJsErrors() {
		ArrayList<JavaScriptError> jsErrors = new ArrayList<JavaScriptError>();
		
		try {
			Object jsErrorsJson = this.executeJavaScript("if(window.self !== window.top) { return []; } else { return JSON.stringify($$eventSniffer.getAndResetJSErrors()); }");
			JSONArray arr = new JSONArray(jsErrorsJson.toString());
			for (int i = 0; i < arr.length(); i++){
				JSONObject current = arr.getJSONObject(i);
				jsErrors.add(new JavaScriptError(current.getInt("linenumber"), current.getString("msg"), current.getString("url")));
			}
	...
	private void executeEventHandler(Eventable eventable) {
		// Assume we only get xpath identifiers for now
		if(eventable.getIdentification().getHow() != Identification.How.xpath)
			LOGGER.error("Only support Xpath as of now.");
		
		String xpath = eventable.getIdentification().getValue();
		String eventType = eventable.getEventType().toString();
		String js = String.format("if(window.self !== window.top) { return []; } else { return $$eventSniffer.simulateEvent('%s', '%s'); }", eventType, xpath);
		this.executeJavaScript(js);
	}
	
	private void executeGesture(String inputType, Eventable eventable) {
		if(eventable.getIdentification().getHow() != Identification.How.xpath)
			LOGGER.error("Only support Xpath as of now.");
		
		String xpath = eventable.getIdentification().getValue();
		String eventType = eventable.getEventType().toString().replace(inputType, "");
		String js = String.format("if(window.self !== window.top) { return []; } else { return $$eventSniffer.gestures.%s.%s('%s'); }", inputType, eventType, xpath);
		this.executeJavaScript(js);
	}
	...
	private boolean fireEventWait(WebElement webElement, Eventable eventable)
		...
		switch (eventable.getEventType()) {
			case click:
				try {
					System.out.println("Attempting click on " + eventable.getIdentification().getValue());
					webElement.click();
			...
			case touchswipeup:
			case touchswipedown:
				executeGesture("touch", eventable);
				break;
			case mouseswiperight:
			case mouseswipeleft:
			case mouseswipeup:
			case mouseswipedown:
				executeGesture("mouse", eventable);
				break;
			default:
				System.out.println("Attempting " + eventable.getEventType().toString() + " on " + eventable.getIdentification().getValue());
				if(webElement.isDisplayed())
					executeEventHandler(eventable);
				else
					return false;
