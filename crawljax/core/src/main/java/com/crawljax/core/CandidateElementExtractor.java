...
public class CandidateElementExtractor {
	...
	private final boolean useEventHandlers;
	...
	public CandidateElementExtractor(ExtractorManager checker, @Assisted EmbeddedBrowser browser,
		...
		useEventHandlers = config.getUseEventHandlers();
	}
	...
	public ImmutableList<CandidateElement> extract(StateVertex currentState)
		...
		try {
			...
			/* Inject code here to get all event handlers registered on page */
			if(useEventHandlers)
				extractEventHandlers(dom, results);
	...
	private void extractEventHandlers(Document dom, List<CandidateElement> results) 
			throws JSONException, XPathExpressionException {
		
		Object events = browser.executeJavaScript("return JSON.stringify($$eventSniffer.eventHandlers);");
		JSONArray arr = new JSONArray(events.toString());
		
		for (int i = 0; i < arr.length(); i++) {
			JSONObject currentItem = arr.getJSONObject(i);
			String xpathExpression = currentItem.getString("xpath");
			Element element = DomUtils.getElementByXpath(dom, xpathExpression);
			
			if(element == null) {
				xpathExpression = xpathExpression.toUpperCase();
				element = DomUtils.getElementByXpath(dom, xpathExpression);
			}
			
			if(element != null) {
				EventType eventType = EventType.click;
				
				String event = currentItem.getString("type"); 
				if(event != "click")
					eventType = EventType.valueOf(event);
				
				switch(eventType) {
					case touchstart:
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchtap));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchdoubletap));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchtaphold));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchswipeleft));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchswiperight));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchswipeup));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.touchswipedown));
						break;
					case mousedown:
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.mouseswiperight));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.mouseswipeleft));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.mouseswipedown));
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", EventType.mouseswipeup));
						break;
					default:
						results.add(new CandidateElement(element, new Identification(Identification.How.xpath, xpathExpression), "", eventType));
						break;	
				}
			}
		}
