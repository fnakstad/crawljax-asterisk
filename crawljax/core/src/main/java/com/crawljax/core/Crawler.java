...
public class Crawler {
	...
	private void crawlThroughActions() {
		System.out.println(stateMachine.getCurrentState().getCandidateElements().size() + " candidate elements in current state");
		...
		while (action != null && !interrupted) {
			...
			if (element.allConditionsSatisfied(browser)) {
				...
				this.browser.getCurrentJsErrors(); // Reset JS errors
	...
	public StateVertex crawlIndex() {
		...
		StateVertex index =
		        vertexFactory.createIndex(url.toString(), browser.getStrippedDom(),
		                stateComparator.getStrippedDom(browser), browser.getCurrentJsErrors());
