...
public class InMemoryStateFlowGraph implements Serializable, StateFlowGraph {
	...
	public boolean addEdge(StateVertex sourceVert, StateVertex targetVert,
		...
		ArrayList<JavaScriptError> jsErrors = targetVert.getJsErrors();
		clickable.setJSErrors(jsErrors);
	...
	StateVertex newStateFor(String url, String dom, String strippedDom, ArrayList<JavaScriptError> jsErrors) {
		int id = nextStateNameCounter.incrementAndGet();
		return vertexFactory.newStateVertex(id, url, getNewStateName(id), dom, strippedDom, jsErrors);
	}
