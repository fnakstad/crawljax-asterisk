...
class OutputBuilder {
	...
	static final String STRIPPED_DOMS_OUTPUT_NAME = "strippedDoms";
	...
	private final File strippedDoms;
	...
	OutputBuilder(File outputDir) {
		...
		strippedDoms = new File(outputDir, STRIPPED_DOMS_OUTPUT_NAME);
		created = strippedDoms.mkdir();
		checkArgument(created, "Could not create stripped doms dir");
	...
	void persistStrippedDom(String name, @Nullable String dom) {
		try {
			Files.write(Strings.nullToEmpty(dom), new File(strippedDoms, name + ".html"), Charsets.UTF_8);
		} catch (IOException e) {
			LOG.warn("Could not save dom state for {}", name);
			LOG.debug("Could not save dom state", e);
       }
